...
export const aiService = {
  async getHint(
    ...
    try {
      ...
      return "Keep coding! Try to break down the problem into smaller steps.";
    }
  },
  ...
  async testConnection(config: AIModelConfig): Promise<boolean> {
    ...
    try {
      ...
      if (config.provider === "gemini") {
        ...
        return (
          typeof result === "string" && result.toLowerCase().includes("ok")
        );
      }
  ...
  async callGemini(
    ...
    try {
      ...
      throw new Error(
        "Failed to generate hint. Please check your Gemini API configuration.",
      );
    }
  },
  ...
  async callGeminiJson(prompt: string): Promise<string> {
    try {
      ...
      throw new Error(
        "Failed to generate question. Please check your Gemini API configuration.",
      );
    }
  },
  ...
  async callProvider(config: AIModelConfig, prompt: string): Promise<string> {
    ...
    try {
      ...
      if (
        provider !== "anthropic" &&
        !url.toLowerCase().includes("/completions")
      ) {
        url = url.replace(/\/+$/, "");
      ...
      throw new Error(
        `Failed to connect to ${provider}. Please check your API configuration.`,
      );
    }
  },
};
