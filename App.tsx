...
const FeedbackAnimation = ({
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-white/40 backdrop-blur-md animate-fade-in">
      <div
        className={`success-bg bg-white p-12 rounded-[4rem] shadow-2xl border flex flex-col items-center animate-sweet-pop ${type === "success" ? "border-pink-100" : "border-red-100"}`}
      >
        {type === "success" ? (
        ...
        <p
          className={`mt-6 text-xl font-black tracking-tighter uppercase ${type === "success" ? "text-pink-500" : "text-red-500"}`}
        >
    ...
    <div className="flex h-screen w-screen bg-[#fffafa] text-gray-800 p-4 gap-4 overflow-hidden fixed inset-0 font-['Quicksand']">
      ...
      <aside className="w-96 bg-white border border-pink-50 flex flex-col shadow-xl rounded-[2.5rem] overflow-hidden shrink-0">
        <div className="p-6 border-b border-pink-50 flex items-center shrink-0">
          <div className="flex items-center gap-2">
            <span className="text-2xl hover:rotate-12 transition-transform cursor-pointer">
              🧁
            </span>
            <h1 className="text-xl font-black text-pink-500 tracking-tighter uppercase">
              Codeling
            </h1>
          </div>
        </div>
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all whitespace-nowrap border ${
                    activeCategory === cat
                      ...
                      : "bg-white text-pink-300 border-pink-50 hover:bg-pink-50"
                  }`}
                >
        ...
        <header className="bg-white h-20 px-8 rounded-[2rem] shadow-sm border border-pink-50 flex items-center justify-between shrink-0">
          <div className="flex bg-gray-50 p-1 rounded-2xl border border-gray-100 shadow-inner">
            <button
              ...
              className={`px-8 py-2.5 rounded-xl text-xs font-black transition-all ${view === "editor" ? "bg-white text-pink-500 shadow-md scale-105" : "text-gray-400 hover:text-gray-600"}`}
            >
              {t.workshop}
            </button>
            <button
              onClick={() => setView("stats")}
              className={`px-8 py-2.5 rounded-xl text-xs font-black transition-all ${view === "stats" ? "bg-white text-pink-500 shadow-md scale-105" : "text-gray-400 hover:text-gray-600"}`}
            >
              {t.dashboard}
            </button>
          </div>
          ...
          {view === "editor" && selectedQuestion ? (
              ...
              <div className="bg-white p-6 px-8 rounded-[2rem] border border-pink-50 flex items-center justify-between shadow-sm shrink-0">
                <div className="flex-1 min-w-0 pr-6">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-[9px] px-2 py-0.5 bg-pink-50 text-pink-400 rounded-md font-black uppercase tracking-widest">
                ...
                <div className="flex items-center gap-4 ml-4 shrink-0 self-start">
                  <div className="flex bg-gray-50 p-1 rounded-xl border border-gray-100 shadow-inner">
                    <button
                      ...
                      className={`px-5 py-1.5 rounded-lg text-[10px] font-black transition-all ${framework === "react" ? "bg-white text-blue-500 shadow-sm" : "text-gray-400"}`}
                    >
                      REACT
                    </button>
                    <button
                      onClick={() => setFramework("vue")}
                      className={`px-5 py-1.5 rounded-lg text-[10px] font-black transition-all ${framework === "vue" ? "bg-white text-green-500 shadow-sm" : "text-gray-400"}`}
                    >
                      VUE
                    </button>
                  </div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-gray-200 animate-fade-in opacity-50">
              ...
              <p className="text-sm font-black uppercase tracking-[0.2em]">
                {lang === "zh" ? "请选择一个挑战" : "Select a challenge"}
              </p>
            </div>
          )}
        </div>
      ...
      {showManualAdd && (
        <div
          ...
          onClick={(e) =>
            closeModalOnBackdrop(e, () => setShowManualAdd(false))
          }
        >
          <div
            className="bg-white rounded-[3rem] p-10 shadow-2xl border border-pink-100 max-w-4xl w-full relative h-[90vh] flex flex-col overflow-hidden animate-sweet-pop"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setShowManualAdd(false)}
              className="absolute top-8 right-10 w-10 h-10 flex items-center justify-center text-gray-300 hover:text-pink-400 transition-colors bg-gray-50 rounded-2xl"
            >
              ✕
            </button>
            ...
            <div className="flex gap-2 mb-8 bg-gray-50 p-1.5 rounded-2xl shrink-0 shadow-inner">
              <button
                onClick={() => setManualTab("info")}
                className={`flex-1 py-3 rounded-xl text-[11px] font-black transition-all ${manualTab === "info" ? "bg-white text-pink-500 shadow-md" : "text-gray-400"}`}
              >
                1. {lang === "zh" ? "信息" : "INFO"}
              </button>
              <button
                onClick={() => setManualTab("react")}
                className={`flex-1 py-3 rounded-xl text-[11px] font-black transition-all ${manualTab === "react" ? "bg-white text-blue-500 shadow-md" : "text-gray-400"}`}
              >
                2. REACT
              </button>
              <button
                onClick={() => setManualTab("vue")}
                className={`flex-1 py-3 rounded-xl text-[11px] font-black transition-all ${manualTab === "vue" ? "bg-white text-green-500 shadow-md" : "text-gray-400"}`}
              >
                3. VUE
              </button>
            </div>
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 min-h-0">
                ...
                <div className="space-y-8 animate-fade-in">
                  <div className="grid grid-cols-2 gap-8">
                    <div className="space-y-3">
                      ...
                      <input
                        ...
                        className="w-full bg-pink-50/20 border border-pink-50 rounded-2xl px-5 py-4 text-sm outline-none shadow-inner focus:border-pink-300 transition-all"
                      />
                    </div>
                    <div className="space-y-3">
                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">
                    ...
                    <div className="flex gap-4">
                      {[
                        ...
                        <button
                          key={d}
                          onClick={() =>
                            setNewQuestion((p) => ({ ...p, difficulty: d }))
                          }
                          className={`flex-1 py-4 rounded-2xl font-black text-[10px] uppercase transition-all border-2 ${newQuestion.difficulty === d ? "bg-pink-50 border-pink-300 text-pink-500 shadow-sm" : "bg-white border-pink-50 text-gray-300"}`}
                        >
                          {t.difficulty[d]}
                        </button>
                      ))}
                  ...
                  <div className="space-y-3">
                    ...
                    <textarea
                      ...
                      rows={6}
                      className="w-full bg-pink-50/20 border border-pink-50 rounded-2xl px-5 py-4 text-sm outline-none shadow-inner focus:border-pink-300 transition-all resize-none font-mono"
                    />
                  </div>
                </div>
              )}
              {manualTab === "react" && (
                <div className="animate-fade-in">
                  <textarea
                    value={newQuestion.react?.initial}
                    onChange={(e) =>
                      setNewQuestion((p) => ({
                        ...p,
                        react: { ...p.react!, initial: e.target.value },
                      }))
                    }
                    rows={12}
                    className="w-full bg-gray-900 text-pink-50 font-mono rounded-2xl px-6 py-5 text-xs outline-none shadow-2xl"
                  />
                </div>
              )}
              {manualTab === "vue" && (
                <div className="animate-fade-in">
                  <textarea
                    value={newQuestion.vue?.initial}
                    onChange={(e) =>
                      setNewQuestion((p) => ({
                        ...p,
                        vue: { ...p.vue!, initial: e.target.value },
                      }))
                    }
                    rows={12}
                    className="w-full bg-gray-900 text-green-50 font-mono rounded-2xl px-6 py-5 text-xs outline-none shadow-2xl"
                  />
                </div>
              )}
            </div>
            <div className="mt-8 pt-8 border-t border-pink-50 flex gap-4 shrink-0">
              <button
                onClick={() => setShowManualAdd(false)}
                className="px-10 py-4 bg-gray-50 text-gray-400 rounded-2xl font-black text-xs uppercase tracking-widest"
              >
                Cancel
              </button>
              <button
                onClick={handleManualSave}
                className="flex-1 py-4 bg-pink-400 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-pink-100 hover:bg-pink-500 transition-all"
              >
                Create Challenge
              </button>
            </div>
      ...
      {showSettings && (
        ...
        >
          <div
            className="bg-white rounded-[3rem] p-10 shadow-2xl border border-pink-100 max-w-2xl w-full h-[85vh] flex flex-col overflow-hidden relative animate-sweet-pop"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex justify-between items-center mb-8 shrink-0">
              ...
              <button
                ...
                className="w-10 h-10 flex items-center justify-center bg-gray-50 text-gray-400 rounded-2xl hover:text-pink-500 transition-all"
              >
                ✕
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-10">
              <div className="space-y-6">
                <p className="text-[10px] font-black text-pink-400 uppercase tracking-widest ml-1">
                  {t.profile}
                </p>
                <div>
                  <label className="text-[10px] font-black text-gray-400 block mb-3 uppercase tracking-widest ml-1">
                    {t.userName}
                  </label>
                  <input
                ...
                {stats.aiConfig?.provider !== "gemini" ? (
                  ...
                  <div className="p-4 bg-pink-50 rounded-2xl border border-pink-100 text-center">
                    ...
                    <p className="text-[10px] text-pink-300 mt-1">
                      {lang === "zh"
                        ? "该提供商已通过系统自动配置。"
                        : "This provider is configured automatically via the system."}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="pt-6 border-t border-pink-50">
                <button
                  onClick={() => {
                    if (confirm(t.clearConfirm)) storageService.clearAllData();
                  }}
