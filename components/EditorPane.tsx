...
import { useEditor, ViewSource } from "../hooks/useEditor";
...
const HistorySidebarItem: React.FC<{
  ...
  return (
    <div
      className={`relative group p-3 rounded-2xl transition-all duration-300 border mb-2 ${
        isActive
          ? "bg-white border-pink-200 shadow-md scale-[1.02]"
          : "bg-white/40 border-transparent hover:border-pink-100 hover:bg-white"
      }`}
    >
      <div
      ...
      >
        ...
        {isEditing ? (
          ...
          <div
            className={`text-[11px] font-bold truncate ${isActive ? "text-pink-600" : "text-gray-600"}`}
          >
      ...
      <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button
          onClick={(e) => {
            e.stopPropagation();
            onUpdate(attempt.id, { isStarred: !attempt.isStarred });
          }}
          className={`p-1 rounded-lg transition ${attempt.isStarred ? "text-amber-400" : "text-gray-300"}`}
        >
          <svg
            className="w-3.5 h-3.5"
            fill={attempt.isStarred ? "currentColor" : "none"}
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
            <path
              ...
              strokeWidth="2.5"
              d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.54 1.118l-3.976-2.888a1 1 0 00-1.175 0l-3.976 2.888c-.784.57-1.838-.197-1.539-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z"
            />
          </svg>
        </button>
        <button
          onClick={(e) => {
            ...
            if (
              confirm(
                lang === "zh" ? "确定删除此历史版本？" : "Delete this version?",
              )
            )
              onDelete(attempt.id);
          }}
...
const EditorPane: React.FC<EditorPaneProps> = ({
  ...
  const {
    ...
    activeCode,
    refreshHistory,
    handleSaveAttempt: _handleSaveAttempt,
    applyToDraft: _applyToDraft,
    isMounted,
  } = useEditor(question, framework, lang, onSave);
  ...
  const generateReactPreview = (code: string) => {
    ...
    const hooksToDestructure = commonHooks.filter(
      (hook) =>
        processedCode.includes(hook) &&
        !processedCode.includes(`function ${hook}`) &&
        !processedCode.includes(`const ${hook}`),
    );

    if (hooksToDestructure.length > 0) {
      processedCode =
        `const { ${hooksToDestructure.join(", ")} } = React;\n` + processedCode;
    }
    ...
    if (framework === "react") {
      ...
      (async () => {
        try {
          ...
          const MonacoEditor = (
            (await import("@vue/repl/monaco-editor")) as any
          ).default;
    ...
    <div className="flex flex-col h-full bg-white rounded-[2.5rem] shadow-2xl border border-pink-100 overflow-hidden relative">
        ...
        <div className="flex items-center gap-4">
          {activeSource === "draft" ? (
            <button
              ...
              className="px-6 py-2 bg-pink-400 text-white rounded-xl text-[11px] font-black hover:bg-pink-500 transition shadow-lg shadow-pink-100 active:scale-95"
            >
              {t.save}
            </button>
          ) : (
            <button
              ...
              className="px-6 py-2 bg-pink-50 text-pink-500 border border-pink-100 rounded-xl text-[11px] font-black hover:bg-pink-100 transition active:scale-95"
            >
          ...
          <div className="flex bg-gray-50 p-1 rounded-xl">
            <button
              ...
              className={`p-1.5 rounded-lg transition ${layoutMode === "both" ? "bg-white text-pink-400 shadow-sm" : "text-gray-300"}`}
            >
              <svg
                className="w-4 h-4"
                fill="none"
            ...
            <button
              ...
              className={`p-1.5 rounded-lg transition ${layoutMode === "editor" ? "bg-white text-pink-400 shadow-sm" : "text-gray-300"}`}
            >
              <svg
                className="w-4 h-4"
                fill="none"
            ...
            <button
              ...
              className={`p-1.5 rounded-lg transition ${layoutMode === "preview" ? "bg-white text-pink-400 shadow-sm" : "text-gray-300"}`}
            >
              <svg
                className="w-4 h-4"
                fill="none"
      ...
      <div className="flex-1 flex overflow-hidden min-h-0 relative bg-white">
        <div
          ...
          style={{
            width: activeSource === "history" ? "256px" : "0px",
            opacity: activeSource === "history" ? 1 : 0,
          }}
        >
          <div className="w-64 flex flex-col h-full shrink-0">
            <div className="p-4 border-b border-pink-50 bg-white/50 flex items-center justify-between">
              <h4 className="text-[10px] font-black text-pink-300 uppercase tracking-widest">
                {t.pastVersions}
              </h4>
              <span className="px-2 py-0.5 bg-pink-100 text-pink-500 rounded-full text-[9px] font-black">
                {history.length}
              </span>
            </div>
            <div className="flex-1 overflow-y-auto custom-scrollbar p-3">
              {history.length === 0 ? (
                <div className="text-center py-12 text-[10px] text-gray-300 font-bold uppercase">
                  {t.noHistory}
                </div>
              ) : (
                history.map((h) => (
                  <HistorySidebarItem
                    ...
                    onUpdate={(id, up) => {
                      storageService.updateAttempt(id, up);
                      refreshHistory();
                    }}
                    onDelete={(id) => {
                      onDeleteAttempt(id);
        ...
        {showSaveNaming && (
          <div
            ...
            onClick={(e) => {
              if (e.target === e.currentTarget) setShowSaveNaming(false);
            }}
          >
            <div
              className="bg-white rounded-[3rem] p-10 shadow-2xl border border-pink-100 max-sm:w-full max-w-sm text-center animate-sweet-pop"
              onClick={(e) => e.stopPropagation()}
            >
              ...
              <div className="flex gap-4">
                <button
                  ...
                  className="flex-1 py-4 bg-pink-400 text-white rounded-[1.5rem] font-black text-xs hover:bg-pink-500 shadow-xl transition active:scale-95"
                >
                  OK
                </button>
                <button
                  onClick={() => setShowSaveNaming(false)}
                  className="flex-1 py-4 bg-gray-100 text-gray-400 rounded-[1.5rem] font-black text-xs hover:bg-gray-200 transition uppercase active:scale-95"
                >
                  Cancel
                </button>
              </div>
